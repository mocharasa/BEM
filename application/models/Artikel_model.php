...
class Artikel_model extends CI_Model{	
	...
	*/
	...
	public function view(){  
		$query 					= "SELECT * FROM artikel JOIN kategori_artikel ON kategori_artikel.id_category=artikel.id_category ORDER BY id_artikel DESC"; 
	...
	function archive(){
		$sql = $this->db->query("SELECT MONTHNAME(tanggal) as bulan, YEAR(tanggal) as tahun FROM artikel AS a GROUP BY tahun DESC");		
        $result = $sql->result();
        return $result;   
	}
	
	function archive2(){
		$sql = $this->db->query("SELECT MONTHNAME(tanggal) as bulan, YEAR(tanggal) as tahun FROM artikel AS a GROUP BY bulan DESC");		
        $result = $sql->result();
        return $result;   
	}
